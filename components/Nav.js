    ...
    <nav className="az-nav">
      ...
      <div className="az-nav-links">
        <Link href="/" className={`az-nav-link${active === 'play' ? ' az-nav-link--active' : ''}`}>
          ...
          <span className="az-nav-link-text">Play</span>
        </Link>
        <Link href="/leaderboard" className={`az-nav-link${active === 'leaderboard' ? ' az-nav-link--active' : ''}`}>
          ...
          <span className="az-nav-link-text">Leaderboard</span>
        </Link>
        <Link href="/friends" className={`az-nav-link${active === 'friends' ? ' az-nav-link--active' : ''}`}>
          ...
          <span className="az-nav-link-text">Friends</span>
        </Link>
      </div>
      ...
      <style jsx global>{`
        ...
        .az-nav-links { display: flex; gap: 4px; }
        .az-nav-link {
          display: flex; align-items: center; gap: 5px;
          padding: 7px 12px; border-radius: 20px; font-size: 0.83rem;
        ...
        .az-signin-btn {
          ...
          padding: 8px 14px; border-radius: 20px; font-size: 0.83rem;
        ...
        .az-nav-user { display: flex; align-items: center; gap: 8px; }
        .az-nav-avatar {
          width: 30px; height: 30px; border-radius: 50%;
          border: 2px solid rgba(0,212,255,0.4); object-fit: cover; flex-shrink: 0;
        }
        ...
        @media (max-width: 520px) {
          .az-nav { padding: 10px 14px; }
          .az-nav-link-text { display: none; }
          .az-nav-link { padding: 8px 10px; }
          .az-nav-name { display: none; }
          .az-signin-btn span { display: none; }
          .az-signout-btn { padding: 6px 10px; font-size: 0.72rem; }
        }
      `}</style>
    </nav>
  );
